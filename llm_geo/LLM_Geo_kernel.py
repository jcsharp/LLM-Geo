...
class Solution:
    ...
    def __init__(
        ...
        graph_requirement.append(
            f"Save the network into GraphML format, save it at: {self.graph_file}"
        )
        graph_requirement_str = "\n".join(
            [f"{idx + 1}. {line}" for idx, line in enumerate(graph_requirement)]
        )
    ...
    def get_LLM_response_for_graph(self, execuate=True):
        ...
        try:
            self.code_for_graph = helper.extract_code(
                response=self.graph_response, verbose=False
            )
        except Exception as e:
            self.code_for_graph = ""
    ...
    def get_ancestor_operations(self, node_name):
        ...
        ancestor_operation_names = [
            ...
            if node_name in self.operation_node_names
        ]
    ...
    def get_descendant_operations(self, node_name):
        ...
        descendant__operation_names = [
            node_name
            for node_name in descendant_node_names
            if node_name in self.operation_node_names
        ]
    ...
    def get_prompt_for_an_opearation(self, operation):
        ...
        descendant_defs = self.get_descendant_operations_definition(
        ...
        operation_prompt = (
            f"Your role: {constants.operation_role} \n\n"
    ...
    def initial_operations(self):
        ...
        for node_name in operation_names:
            function_def_returns = helper.generate_function_def(
                node_name, self.solution_graph
            )
    ...
    def get_LLM_responses_for_operations(self, review=True):
        ...
        for idx, operation in enumerate(self.operations):
            ...
            try:
                operation_code = helper.extract_code(
                    response=operation["response"], verbose=False
                )
    ...
    def prompt_for_assembly_program(self):
        all_operation_code_str = "\n".join(
            [operation["operation_code"] for operation in self.operations]
        )
        ...
        assembly_requirement = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.assembly_requirement)
            ]
        )

        assembly_prompt = (
            f"Your role: {constants.assembly_role} \n\n"
    ...
    def get_LLM_assembly_response(self, review=True):
        ...
        assembly_LLM_response = helper.get_LLM_reply(
            self.assembly_prompt,
        ...
        try:
            code_for_assembly = helper.extract_code(
                response=self.assembly_LLM_response, verbose=False
            )
        except Exception as e:
            code_for_assembly = ""
    ...
    def direct_request_prompt(self):
        direct_request_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.direct_request_requirement)
            ]
        )
    ...
    def get_direct_request_LLM_response(self, review=True):
        response = helper.get_LLM_reply(
            prompt=self.direct_request_prompt,
            model=self.model,
            stream=self.stream,
    ...
    def execute_complete_program(self, code: str, try_cnt: int = 10) -> str:
        ...
        while count < try_cnt:
            ...
            #
                ...
                if count == try_cnt:
                    print(
                        f"Failed to execute and debug the code within {try_cnt} times."
                    )
                    return code
    ...
    def get_debug_prompt(self, exception, code):
        ...
        debug_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.debug_requirement)
            ]
        )
    ...
    def ask_LLM_to_review_operation_code(self, operation):
        ...
        review_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.operation_review_requirement)
            ]
        )
        review_prompt = (
            f"Your role: {constants.operation_review_role} \n"
    ...
    def ask_LLM_to_review_assembly_code(self):
        ...
        review_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.assembly_review_requirement)
            ]
        )
        review_prompt = (
            f"Your role: {constants.assembly_review_role} \n"
    ...
    def ask_LLM_to_review_direct_code(self):
        ...
        review_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.direct_review_requirement)
            ]
        )
        review_prompt = (
    ...
    def ask_LLM_to_sample_data(self, operation_code):
        sampling_data_requirement_str = "\n".join(
            [
                f"{idx + 1}. {line}"
                for idx, line in enumerate(constants.sampling_data_requirement)
            ]
        )
        ...
        # print(f"review_prompt:\n{review_prompt}")
        response = helper.get_LLM_reply(
            prompt=sampling_data_review_prompt,
